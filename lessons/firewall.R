...
fileList = list.files("lessons", pattern="^1[12].*qmd$");
fileText = c();

# read in each file
for(i in 1:length(fileList))
{
  if( !(grepl("_fw", fileList[i]) ) )
  {
    # read every line of file
    fileText[i] = read.delim(file = paste("lessons/", fileList[i], sep=""), 
                             header = FALSE,
                             blank.lines.skip = FALSE,  # include blank lines
                             quote = "");               # don't replace quotes
    
    
    newFileText = gsub("plot", "_plot_", fileText[[i]])
    
    newFile = gsub("\\.", "_fw\\.", fileList[i])
    write(newFileText, paste("lessons/", newFile, sep=""))
  }
}
